...
double* simulation(std::string datarate)
{
  ...
  p2p.SetDeviceAttribute ("DataRate", StringValue ("1000kb/s"));
  ...
  // Create the OnOff application for clearly defined demand in traffic
    ...
    onoff.SetAttribute ("Remote",
                        AddressValue (InetSocketAddress (wifiInterfaces.GetAddress (2), port)));
    apps = onoff.Install (p2pNodes.Get (1));
    apps.Start (Seconds (1.0));
    apps.Stop (Seconds (10.0));
...
int main (int argc, char *argv[])
{
  ...
  std::string fileNameWithNoExtension = "up1down2User";
